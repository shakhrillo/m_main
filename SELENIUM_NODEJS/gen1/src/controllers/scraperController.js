...
async function main(url, uid, pushId, isDev) {
  try {
    ...
    page.on("request", (request) => {
      ...
      if (
        disabledRequests.some((disabledRequest) =>
          request.url().includes(disabledRequest)
        )
      ) {
        request.abort();
      } else {
    ...
    const allElements =
      (await scrollAndCollectElements(page, firestore, uid, pushId)) || [];
    ...
    if (!isDev) {
      let collectionReviews = firestore.collection(
