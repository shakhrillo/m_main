...
export const login = async (email: string, password: string) => {
  if (!auth) {
    return new Promise((resolve, reject) =>
      reject("Firebase auth is not available"),
    )
  }
  if (!email || !password) {
    return new Promise((resolve, reject) =>
...
export const startExtractGmapReviewsOverview = async (
  uid: string,
  data: {
    url: string
  },
) => {
  const collectionReviews = collection(firestore, `users/${uid}/reviewOverview`)
