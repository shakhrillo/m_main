...
export const SingleReview = () => {
  ...
  useEffect(() => {
    ...
    const subscription = dockerContainers({
      ...
      .subscribe((data) => setContainer(data?.location ? data : ({} as IDockerContainer)));

    return () => subscription.unsubscribe();
  }, [reviewId, user]);

  const renderTab = (eventKey: string, Icon: any, title: string, count?: number, Component?: any) => (
    <Tab eventKey={eventKey} title={<Stack direction="horizontal" gap={2}><Icon size={20} />{title}{count && <Badge>{count}</Badge>}</Stack>}>
  ...
  return (
    <Container>
      <Breadcrumb>
        <Breadcrumb.Item onClick={() => navigate("/reviews")}>Reviews</Breadcrumb.Item>
        <Breadcrumb.Item active>{container.title}</Breadcrumb.Item>
      </Breadcrumb>
      <Row>
        <Col md={9}>
          <Tabs variant="pills" defaultActiveKey="comments">
            {renderTab("comments", IconMessage, "Comments", container.totalReviews, CommentsList)}
            {renderTab("images", IconLibraryPhoto, "Images", container.totalImages, ImagesList)}
            {renderTab("videos", IconVideo, "Videos", container.totalVideos, VideosList)}
          </Tabs>
        </Col>
        <Col md={3}>
