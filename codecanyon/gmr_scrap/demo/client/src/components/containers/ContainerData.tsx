        ...
        path === "containers" && (
          <>
            <Stack direction="horizontal" gap={2} className="text-muted">
              ...
              {(container?.machine?.Actor?.Attributes?.execDuration || 0) + "s exec duration"}
            </Stack>
          </>
        )
      }
            ...
            <Ratings container={container} />
            {
              container.type === "comments" && (
                <Stack direction="horizontal" gap={2} className="text-muted">
                  ...
                  <div>{formatNumber(container.totalOwnerReviews)} Owner Reviews</div>
            ...
            }
            {
              container.type === "info" && (
                <Stack direction="horizontal" gap={2} className="text-muted">
                  ...
                  <div>Extract Images: {container.extractImageUrls ? "Yes" : "No"}</div>
                  <div>Extract Videos: {container.extractVideoUrls ? "Yes" : "No"}</div>
                  <div>Extract Owner Response: {container.extractOwnerResponse ? "Yes" : "No"}</div>
                </Stack>
              )
            }
          </>
