...
async function getOwnerResponse(reviewElement) {
  ...
  try {
    ...
    let starRating = await reviewElement.$$(
    ...
    const reviewContainer = await starRating.evaluateHandle((el) => {
      if (!el) return null;
    ...
    const responseSection = await reviewContainer.evaluateHandle(
    ...
    const responseSectionText = await responseSection.evaluate((el) => {
      if (!el) return "";
    ...
    if (responseSectionText.includes("Response from the owner")) {
      ...
      if (responseContentDivs.length > 1) {
        ...
        ownerResponse = await responseContent.evaluate((el) =>
          el.textContent.trim()
        );
      } else {
