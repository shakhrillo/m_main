...
async function clickReviewTab(page, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      await page.waitForSelector('button[role="tab"]', { visible: true, timeout: 3000 });
      ...
      for (const button of allButtons) {
        const tabText = await button.evaluate(el => el.textContent?.trim().toLowerCase() || '');
      ...
      logger.warn(`Attempt ${attempt} to click the review tab failed: ${error.message}`);
      if (attempt < retries) {
